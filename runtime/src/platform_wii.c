/*
 * platform_wii.c — Nintendo Wii backend for the gb-recompiled runtime.
 *
 * Implements the platform_sdl.h contract + GBPlatformCallbacks (gbrt.h),
 * against libogc (devkitPPC toolchain). Plain C.
 *
 * Wii is PowerPC big-endian — the AF/BC/DE/HL register-pair fix in
 * gbrt.h (Phase 5a) makes the recompiled rom.c correct here.
 *
 * Subsystems:
 *   gccore / VIDEO — the external framebuffer (XFB). The Wii XFB is
 *     YUY2 (2 pixels per 32-bit word); render_frame converts the GB
 *     ARGB framebuffer to YUY2.
 *   wiiuse/wpad — Wii Remote (held sideways)
 *   ogc/pad     — GameCube controller
 *   asndlib     — audio (44.1 kHz stereo, matches the GB runtime)
 *   fat         — SD-card saves
 *
 * Output: a .dol — runs in Dolphin and via the Homebrew Channel.
 */
...
/* --- RGB -> YUY2 (the Wii XFB stores two pixels per 32-bit word) ------- */
...
/* Fill one ASND buffer from the ring (silence-pad on underrun). */
...
/* ASND voice callback — queue the next buffer when the voice drains. */
...
static void on_power(void)               { g_quit = 1; }
...
void gb_platform_render_frame(const uint32_t* framebuffer)
{
    ...
    for (int gy = 0; gy < GB_H; ++gy) {
        for (int sy = 0; sy < g_scale; ++sy) {
            ...
            /* Walk the GB row as pixel pairs across the scaled output. */
            for (int gx = 0; gx < GB_W; gx += 2) {
                uint32_t p0 = src[gx];
                uint32_t p1 = (gx + 1 < GB_W) ? src[gx + 1] : p0;
                /* Each GB pixel is g_scale wide; emit the pair scaled. */
                int base = g_off_x + gx * g_scale;
                for (int s = 0; s < g_scale; ++s) {
                    int wx0 = (base + s) / 2;          /* left  px word */
...
bool gb_platform_init(int scale)
{
    ...
    /* clear the whole XFB to black (YUY2 black = Y0 Cb128 Y0 Cr128) */
    uint32_t black = rgb2yuy2(0, 0);
    for (int i = 0; i < (g_fbw / 2) * g_fbh; ++i) g_xfb[i] = black;

    /* largest integer scale of 160x144 that fits */
...
bool gb_platform_poll_events(GBContext* ctx)
{
    ...
    /* ---- Wii Remote, port 0, held sideways ----
     * Sideways the d-pad rotates 90° CW: physical-up = WPAD RIGHT, etc. */
    WPAD_ScanPads();
    uint32_t wm = WPAD_ButtonsHeld(0);
    if (wm & WPAD_BUTTON_RIGHT) dpad &= ~0x04;  /* up    */
    if (wm & WPAD_BUTTON_LEFT)  dpad &= ~0x08;  /* down  */
    if (wm & WPAD_BUTTON_UP)    dpad &= ~0x02;  /* left  */
    if (wm & WPAD_BUTTON_DOWN)  dpad &= ~0x01;  /* right */
    if (wm & WPAD_BUTTON_2)     btns &= ~0x01;  /* A */
    if (wm & WPAD_BUTTON_1)     btns &= ~0x02;  /* B */
    if (wm & WPAD_BUTTON_MINUS) btns &= ~0x04;  /* Select */
    if (wm & WPAD_BUTTON_PLUS)  btns &= ~0x08;  /* Start  */
    if (wm & WPAD_BUTTON_HOME)  g_quit = 1;
...
#define STATE_MAGIC   0x4C413357u   /* "LA3W" */
